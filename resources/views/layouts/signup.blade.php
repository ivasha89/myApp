...
<div class="card-body">
    <div class="form-group">
        ...
        <input type="text" class="form-control {{ $errors->has('nm') ? 'is-danger' : ''}}" id="name" placeholder="{{ old('nm') }}"  name="nm" autocapitalize="on" required autofocus>
    </div>
    <div class="form-group">
        <label class="text-info" for="psw">
            Пароль
        </label>
        <input type="password" class="form-control {{ $errors->has('ps') ? 'is-danger' : ''}}" id="psw" name="ps" maxlength="16" required>
    </div>
    <div class="form-group">
        <label class="text-info" for="sn">
            Духовное имя
        </label>
        <input type="text" class="form-control {{ $errors->has('sn') ? 'is-danger' : ''}}" id="sn" placeholder="{{ old('sn') }}" name="sn" autocapitalize="on">
    </div>
    <div class="form-group">
